...
.MODULE $M.main;
   ...
   .CONST  $AUDIO_LEFT_IN_PORT    ($cbuffer.READ_PORT_MASK + $cbuffer.FORCE_SIGN_EXTEND + 0);
   .CONST  $AUDIO_RIGHT_IN_PORT   ($cbuffer.READ_PORT_MASK + $cbuffer.FORCE_SIGN_EXTEND + 1);
   .CONST  $AUDIO_LEFT_OUT_PORT   ($cbuffer.WRITE_PORT_MASK + $cbuffer.FORCE_SIGN_EXTEND + 0);
   .CONST  $AUDIO_RIGHT_OUT_PORT  ($cbuffer.WRITE_PORT_MASK + $cbuffer.FORCE_SIGN_EXTEND + 1);
   ...
   .VAR/DMCIRC $user_code.fir_filter.filter_coefs[$FILTER_LENGTH] = 
                                                                    #include "fir_hp_coeffs.dat"
                                                                    ;
   // Initialize a circular buffer for filter coefficients
   .VAR/DMCIRC $user_code.fir_filter.filter_data_buffer_left[$FILTER_LENGTH];
   .VAR/DMCIRC $user_code.fir_filter.filter_data_buffer_right[$FILTER_LENGTH];
   ...
   .BLOCK $audio_in_left_op;
      ...
      .VAR $audio_in_left_op.param[$user_code.fir_filter.STRUC_SIZE] =
         0,                                             // Input index
         2,                                             // Output index
         &$user_code.fir_filter.filter_coefs,           // Coefficient Buffer Address
         &$user_code.fir_filter.filter_data_buffer_left,    // History Buffer Address
   ...
   .BLOCK $audio_in_right_op;
      ...
      .VAR $audio_in_right_op.param[$user_code.fir_filter.STRUC_SIZE] =
         ...
         3,                            // Output index
         &$user_code.fir_filter.filter_coefs,           // Coefficient Buffer Address
         &$user_code.fir_filter.filter_data_buffer_right,   // History Buffer Address
         $FILTER_LENGTH,                                // Filter Length
         0;                            // Shift amount
   .ENDBLOCK;
  #else
   ...
   // occuring within the timer period. The easiest way to do this is to
   ...
   wait_for_adc_buffer_wraparound:
    ...
    Null = r0 - 32;                    // calc_amount_data r0 returns number of words/samples
   ...
   // Start a loop to copy the data from the input through to the output buffers
   copy_loop:
