...
#define $APTX_ACL_SPRINT_TMR_PERIOD_AUDIO_COPY      625    // timer period to read the audio data in analogue mode
...
.MODULE $M.main;
  ...
  .VAR $audio_in_src_struct[$src.upsample_downsample.STRUC_SIZE] = 
   ...
   .VAR $mv.codec_type = -1;
   .VAR $vm.codec_type.audio_timer_period =  $APTX_ACL_SPRINT_TMR_PERIOD_AUDIO_COPY; 
   ...
   // -------------------------------------
   //  function that runs for ADC mode
   //
   //  just ENCODER
   // -------------------------------------
   analogue_function:
...
.MODULE $M.codec_in_copy_handler;
   ...
   // received frame is removed, in normal operation this doesnt happen, but if for any reason
